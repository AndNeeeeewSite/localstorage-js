...
function render(){
    ...
    for(let item of deleteButtons){
        item.addEventListener('click',function(){
           buttLi = item.parentElement.children
...
addBut.addEventListener('click', function(){
    ...
    if(addInputText == '' || addInputText1 == '' || addInputText2 == '' || addInputText3 == ''){
        alert('You must write url')
    }
    else{
        urlsJSON.urls.push([addInputText,addInputText1,addInputText2,addInputText3])
        localStorage.setItem('urls', JSON.stringify(urlsJSON))
        console.log(urlsJSON)
        clearList()
        render()
